...
import modal from '../../finance/components/AddEditQuote.vue'
export default {
  data() {
    return {
    invoicesShow: [],
  ...
  methods: {
    total(row) {
      ...
      let discount = 0;
      let tax = 0;
      for(let i = 0; i<row.quoteItems.length; i++) {
        total += Number(row.quoteItems[i].qty) * Number(row.quoteItems[i].unitPrice);
      }
     ...
     return Math.round(total) ;
      ...
      ...mapActions('financeStore', ['deleteQuote']),
      rowColor(props) {
