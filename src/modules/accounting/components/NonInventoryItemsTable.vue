          ...
          <q-btn dense round flat icon="more_vert">
            ...
            >
              <q-list style="min-width: 75px">
                <q-item  @click="delNItem(props.row.id)" style="padding 0 !important" clickable v-close-popup>
...
import modal from './AddEditNItems.vue'
export default {
  data() {
    return {
    prompt: false,
  ...
  computed : {
      ...mapState('accountingStore', ['nonInventoryItems'])
  },
  methods: {
      ...mapActions('accountingStore', ['getNonInventoryItems','delNItem']),
      rowColor(props) {
...
async mounted() {
     await this.getNonInventoryItems()
  }
}
</script>
