      ...
      <template v-slot:body="props">
        <q-tr :class="props.rowIndex%2 !== 0 && 'bg-white'" :props="props">
         ...
         <q-td key="actions" :props="props">
          <q-btn v-if="canActivate('subject_finance','update') || canActivate('subject_finance','delete')" :disable="props.row.status === 'Paid'" dense round flat icon="more_vert">
            <q-menu
...
import modal from './AddEditInvoice.vue'
export default {
  props: ['type'],
  data() {
    return {
  ...
  computed : {
      ...mapState('financeStore', ['sentInvoices','paidInvoices','notPaidInvoices']),
      data() {
        if(this.type === 'Sent') {
