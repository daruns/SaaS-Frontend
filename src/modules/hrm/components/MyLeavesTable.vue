...
export default {
  computed: {
    ...
    data(){
      ...
      let finalres = JSON.parse(JSON.stringify(this.myLeaves)).sort((a,b) => b['id'] - a['id'])
      let finalresParsed = finalres.map(leave => {
        if (leave.leaveApprovals.some(approval => {return approval.manager.hrMember === 1})) {
          ...
          console.log("copy leaves: ", copleave)
          return copleave
        }
      })
  ...
  methods: {
    ...mapActions('hrmStore',['getMyLeaves','deleteLeave']),
