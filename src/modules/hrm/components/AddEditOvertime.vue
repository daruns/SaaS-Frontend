    ...
    <q-card style="max-width: 500px;min-height:100vh;" flat square>
      ...
      <q-card-section class="q-gutter-sm scroll">
        ...
        <q-checkbox
          ...
          v-model="forMe"
          label="My overtime (Overtime Request for my self)"
        />
        <q-select
          v-model="employee"
        ...
        <div class="flex col-12">
          ...
          >
            ...
            <div v-if="fromtToovertimetypeDaysExceedToggle" class="text-red">* Days exceeded the Overtime Type limit</div>
          </q-date>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      fromtToovertimetypeDaysExceedToggle: false,
      loading: false,
      fromtToToggle: false,
      overtimeTypeDuration: 'hours',
      overtimeTypeOptions: [],
      employeeOptions: [],
      employee: null,
      currentBalance: 0,
  ...
  computed: {
    ...mapState('hrmStore', ['allEmployees', 'overtimeTypes']),
  },
  methods: {
    ...mapActions('hrmStore', ['createOvertime', 'updateOvertime','getOvertimes', 'getEmployees', 'getOvertimeTypes']),
    trigchkbxfrme(targval) {
      ...
      if (targval === true) {
        if (this.currentUser.myEmployeeProfile) {
          this.currentBalance = Number(this.currentUser.myEmployeeProfile.overtimeBalance)
    ...
    setCurrentBalance(val) {
      ...
      this.currentBalance = Number(val?.overtimeBalance)
      this.remainBalance = Number(val?.overtimeBalance - difference)
      this.setRemainingDays({from: this.credentials.from, to: this.credentials.to})
    },
    setRemainingDays(val) {
      if (val === null) val = {}
      console.log("typeof: ",typeof val, val, "overtimetype: ",this.overtimeType.durationType)
      if (typeof val === 'object' && !Object.values(val).includes(null) && Object.keys(val).includes('from')) {
        let vfrom = val?.from
        let vto = val?.to
        if (this.overtimeType.durationType === "hours") {
      ...
      this.fromtToovertimetypeDaysExceedToggle = (Number(this.currentBalance) - Number(this.remainBalance)) > Number(this.overtimeType?.hours)
      console.log("balance: " + Number(this.currentBalance) , "remain: " + Number(this.remainBalance),"overtimetype: " +  Number(this.overtimeType?.hours) )
      this.fromtToDaysExceedToggle = (Number(this.remainBalance) < 0) && this.overtimeType?.urgent != true
    ...
    async submit() {
      ...
      if ((Number(this.currentBalance) - Number(this.remainBalance)) > Number(this.overtimeType.hours)) {
        this.fromtToovertimetypeDaysExceedToggle = true
      }
      if (Number(this.remainBalance) < 0 && this.overtimeType?.urgent != true) {
        this.fromtToDaysExceedToggle = true
      }
      this.$refs.overtimeTypeRef.validate();
      if (!this.currentUser?.myEmployeeProfile) {
      ...
      if (
        this.$refs.overtimeTypeRef?.hasError
        ||
        this.credentials.from === null
        ||
        this.credentials.to === null
        ...
        ||
        ((Number(this.currentBalance) - Number(this.remainBalance)) > Number(this.overtimeType.hours))
        ||
        (Number(this.remainBalance) < 0 && !this.overtimeType?.urgent == true)
      ) {
        return
      }else{
        this.loading = true
        let data = {
          ...
          overtimeTypeId: Number(this.overtimeType?.id),
        };
        ...
        if(this.actionType === 'Add'){
          if (this.overtimeType) {
            data['overtimeTypeId'] = Number(this.overtimeType.id)
          }
          if (this.employee) {
            if (!this.currentUser.myEmployeeProfile) {
              data['employeeId'] = Number(this.employee.id)
          ...
          let selfout = this
          try {
            await this.createOvertime(data)
            selfout.$q.notify({
              type: 'positive',
            ...
            selfout.$q.notify({
              type: 'negative',
              message: 'Something went wrong while requesting Overtime!',
              color: 'negative',
              position: 'top',
              timeout: '500'
            })
          ...
          await this.updateOvertime(data);
          this.getOvertimes();
          selfout.$q.notify({
            type: 'positive',
  ...
  async mounted() {
    if (this.actionType === 'Add') {
      this.employee = {label: this.currentUser.myEmployeeProfile.name, id: this.currentUser.myEmployeeProfile.id, overtimeBalance: this.currentUser.myEmployeeProfile.overtimeBalance}
      if (this.currentUser.myEmployeeProfile) {
        this.currentBalance = Number(this.currentUser.myEmployeeProfile?.overtimeBalance)
        this.remainBalance = this.currentBalance
      }
      this.overtimeTypeLoading = true
      this.getValuesFromApiAsync(this.getOvertimeTypes()).then(() => {
        for (let overtimeType of this.overtimeTypes) {
          this.overtimeTypeOptions.push({label: `${overtimeType.name} - ${Number(overtimeType.hours)} ${overtimeType.durationType}${overtimeType.urgent ? ' - Urgent' : ""}`, id: overtimeType.id,hours: overtimeType.hours,urgent: overtimeType.urgent,durationType: overtimeType.durationType})
        }
        this.overtimeTypeLoading = false
      })
      this.employeeLoading = true
      this.getValuesFromApiAsync(this.getEmployees()).then(() => {
        for (let employee of this.allEmployees) {
          this.employeeOptions.push({label: employee.name, id: employee.id, overtimeBalance: employee.overtimeBalance})
        }
    ...
    if (this.actionType === 'Edit') {
      ...
      this.getValuesFromApiAsync(this.getOvertimeTypes()).then(() => {
        ...
        this.overtimeType = {label: `${this.body.overtimeType.name} - ${Number(this.body.overtimeType.hours)} ${overtimeType.durationType}${this.body.overtimeType.urgent ? ' - Urgent' : ""}`, id: this.body.overtimeType.id,hours: this.body.overtimeType.hours,urgent: this.body.overtimeType.urgent,durationType: overtimeType.durationType};
        this.overtimeTypeLoading = false
        this.employee = {label: this.currentUser.myEmployeeProfile.name, id: this.currentUser.myEmployeeProfile.id, overtimeBalance: this.currentUser.myEmployeeProfile.overtimeBalance}
        if (this.currentUser.myEmployeeProfile) {
          this.currentBalance = Number(this.currentUser.myEmployeeProfile?.overtimeBalance)
      ...
      if (this.currentUser.myEmployeeProfile) {
        ...
        this.getValuesFromApiAsync(this.getEmployees()).then(() => {
          ...
          this.employee = {label: this.body.employee?.name, id: this.body.employee?.id, overtimeBalance: this.body.employee?.overtimeBalance}
          this.currentBalance = this.body.employee?.overtimeBalance
          this.employeeLoading = false
        })
      }
    }
    console.log("overtimetpye----",this.employee)
  }
}
</script>