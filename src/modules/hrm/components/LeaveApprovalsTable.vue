<template>
  <div>
    <h6 class="q-pa-md">Pending Approval Leaves</h6>
    <q-table
      :rows="data"
      :columns="columns"
          ...
          <q-td class="text-right" key="actions" :props="props">
            <div v-if="currentUser?.myEmployeeProfile?.hrMember === 1" class="row" style="min-width: 75px">
              <q-btn
                ...
                @click="approveLeaveMiddle({id: props.row.id, status: 'completed'})"
                flat
                no-caps 
                :disable="(props.row.isRejectCompleted || props.row.leaveApprovals.some(ee => ee.status === 'rejected') || props.row.status === 'rejected')">
              ...
              <q-btn
                ...
                no-caps 
                :disable="(props.row.isRejectCompleted || props.row.leaveApprovals.some(ee => ee.status === 'rejected') || props.row.status === 'rejected')">
                <q-icon class="text-bold" name="close" :color="(props.row.isRejectCompleted || props.row.leaveApprovals.some(ee => ee.status === 'rejected') || props.row.status === 'rejected') ? 'grey' : 'red'" size="xs"></q-icon>Reject hr
              </q-btn>
            </div>
...
export default {
  computed: {
    ...mapState('hrmStore', ['leaveApprovals']),
    data(){
      let finalres = JSON.parse(JSON.stringify(this.leaveApprovals)).sort((a,b) => b['id'] - a['id'])
      let finalresParsed = finalres.map(leave => {
        ...
        if (leave.leaveApprovals.some(approval => {return approval.manager.hrMember === 1})) {
          ...
          copleave['isRejectCompleted'] = this.checkInApprovals(copleave.leaveApprovals)
          return copleave
        }
      })
      console.log("leave approvals: ",finalresParsed)
  ...
  methods: {
    ...mapActions('hrmStore',['getLeaveApprovals','updateApproval','approveLeave']),
    ...mapActions('example', ['getUser']),
    async approveLeaveMiddle (datt) {
      if (datt.status === 'rejected') {
        this.rejectLoading = datt.id;
        await this.approveLeave(datt);
